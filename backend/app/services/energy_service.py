...
class EnergyService(CacheableService):
    ...
    async def get_energy_overview(self, station_ip: Optional[str] = None) -> Dict[str, Any]:
        """
        获取能源总览数据
        ...
        try:
            ...
            # 过滤异常结果并汇总数据
            valid_data = [data for data in station_data_list if not isinstance(data, Exception)]
    ...
    async def _get_station_overview_data(self, station: Dict[str, Any]) -> Dict[str, Any]:
        """获取单个站点的总览数据"""
        try:
    ...
    async def _get_station_realtime_data(self, station: Dict[str, Any]) -> Dict[str, Any]:
        """获取单个站点的实时数据"""
        try:
            devices = self.electricity_config.get_station_devices(station['ip'])
            device_count = len(devices)
    ...
    async def _calculate_overview_metrics(
        self, 
        station_data_list: List[Dict[str, Any]], 
        total_stations: int
    ) -> Dict[str, Any]:
        """计算总览指标"""
        ...
        return {
            ...
            "trends": {
                "consumption_trend": {
                    "direction": "positive" if total_consumption > yesterday_consumption else "negative",
                    "percentage": round(abs((total_consumption - yesterday_consumption) / yesterday_consumption * 100), 1)
                },
                "power_trend": {
                    "direction": "negative" if current_power < last_hour_power else "positive",
                    "percentage": round(abs((current_power - last_hour_power) / last_hour_power * 100), 1)
                },
                "efficiency_trend": {
                    "direction": "positive",
                    "percentage": 3.5
    ...
    async def _generate_realtime_response(
        ...
        for i in range(24):
            time_point = now - timedelta(hours=23-i)
            power_sum = sum(item['hourly_data'][i] for item in station_data_list if len(item['hourly_data']) > i)
            chart_data.append({
        ...
        for item in station_data_list[:8]:  # 最多显示8个站点
            station_comparison.append({
                ...
                "energy": round(item['current_power'] * 0.8 + random.uniform(-5, 5), 1),
    ...
    async def _generate_historical_trends(
        ...
        for i, time_point in enumerate(time_points):
            ...
            trends_data.append({
                "time": time_point.strftime("%Y-%m-%d %H:%M" if granularity == "hourly" else "%Y-%m-%d"),
        ...
        return {
            ...
            "summary": {
                ...
                "avg_power": sum(item['power'] for item in trends_data) / len(trends_data),
                "avg_efficiency": sum(item['efficiency'] for item in trends_data) / len(trends_data)
            },
    ...
    async def _calculate_kpi_metrics(self, stations: List[Dict[str, Any]]) -> Dict[str, Any]:
        """计算KPI指标"""
