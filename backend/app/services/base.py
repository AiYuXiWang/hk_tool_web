...
class BaseService(ABC, Generic[T]):
    ...
    def log_error(self, operation: str, error: Exception, **kwargs) -> None:
        """记录错误日志"""
        self.logger.error(
            f"操作失败: {operation} - {str(error)}", exc_info=True, extra=kwargs
        )
...
class CacheableService(BaseService[T]):
    ...
    async def set_cache(
        self, key: str, value: Any, timeout: Optional[int] = None
    ) -> None:
        """设置缓存数据"""
        if not self.cache_enabled:
            return
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    self.logger.warning(f"操作失败，第{attempt + 1}次重试: {str(e)}")
...
def service_method(cache_timeout: Optional[int] = None):
    ...
    def decorator(func):
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                # 记录操作开始
                self.log_operation(operation_name, call_args=args, call_kwargs=kwargs)
                ...
                self.log_error(operation_name, e, call_args=args, call_kwargs=kwargs)
                raise
