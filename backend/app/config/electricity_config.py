...
class ElectricityConfig:
                ...
                if station_config.get("ip") == station_ip:
                    ...
                    for device in devices:
                        standardized_device = {
                            ...
                            "location": device.get("location", "未知位置"),
                            "power": device.get("power", 0),
                            "ct_ratio": device.get("ct_ratio", 1),
    ...
    def _extract_line_code(self, line_name: str) -> str:
        """从线路名称中提取线路代码"""
        normalized = (line_name or "").strip()
        if not normalized:
            return "M0"

        normalized_upper = normalized.upper()
        if normalized_upper.startswith("M") and normalized_upper[1:].isdigit():
            return normalized_upper

        if "3号线" in normalized:
            return "M3"
        elif "8号线" in normalized:
            return "M8"
        elif "11号线" in normalized:
            return "M11"
        elif "1号线" in normalized:
            return "M1"
        elif "2号线" in normalized:
            return "M2"
        else:
            # 尝试从名称中提取数字
            import re
