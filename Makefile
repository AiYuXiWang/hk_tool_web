# 海康威视工具平台 - 开发工具集
.PHONY: help install dev build test lint format clean spec-validate spec-new-api spec-new-model spec-new-component
...
# Spec Coding 相关命令
spec-validate: ## 验证规格一致性
    @echo "验证 API 规格一致性..."
    @python tools/spec_validation/validate_api_spec.py

spec-new-api: ## 创建新的 API 规格文档
    @echo "创建 API 规格文档..."
    @python tools/create_spec.py --type api

spec-new-model: ## 创建新的数据模型规格文档
    @echo "创建数据模型规格文档..."
    @python tools/create_spec.py --type model

spec-new-component: ## 创建新的组件规格文档
    @echo "创建组件规格文档..."
    @python tools/create_spec.py --type component